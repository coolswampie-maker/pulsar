...
class Company(models.Model):
    ...
    #
    # Численности и выручки здесь намеренно нет, хотя программы ограничивают
    # и их: эти числа резидент уже сдаёт в разделе «Показатели» — по годам и
    # с подтверждающими документами. Вторые такие же поля означали бы два
    # числа об одном и том же, которые неизбежно разойдутся, и проверка
    # отвечала бы уверенно, но по устаревшим данным. Проверка берёт их
    # из Kpi (см. formal._kpi).
